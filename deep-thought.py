    ...
    if isinstance(filepath, bool) and filepath == False:
        ...
        try:
            if not os.path.splitext(filepath)[1] == FILE_EXTENSION:
                _ = os.path.splitext(sys.argv[0])[0]+FILE_EXTENSION
            else:
                _ = filepath
        except IndexError:
            _ = os.path.splitext(sys.argv[0])[0]+FILE_EXTENSION
...
def set_seed_from_user(the_input):
    if isinstance(the_input, str) and not the_input == "":
        proc_input = 0
        for i in the_input:
...
def parse_raw(info):
    the_string = ''
    switch = train.d_list
    for i,_ in enumerate(switch):
        _ = nth_keyname(switch,i)
        if not _ is False:
            if info.find(_) > -1:
                return switch[_]
    return info
